...
//均值
if ( ! function_exists( 'avg_sum' ) ) {
    function avg_sum( $data ) {
        return array_sum($data)/count($data);
    }
}

//各个数据统计
if ( ! function_exists( 'each_count' ) ) {
    function each_count( $data ) {
        //症状全部转为数组
        foreach ($data as $k =>$v) {
            $data[$k] = explode(',',$v);
        ...
        //症状统计
        return array_count_values($each_count);
    }
}


//数据分割
if ( ! function_exists( 'data_exp' ) ) {
    function data_exp( $data ) {
        foreach ($data as $k =>$v) {
            $data[$k] = explode(',', $v);
        }
        return $data;
